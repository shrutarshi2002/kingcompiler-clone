...
const Chatbot = () => {
  ...
  const [showWhatsAppPopup, setShowWhatsAppPopup] = React.useState(true);
    ...
    return (
      ...
      >
        ...
        <div
          ...
          onMouseLeave={() => {
            hideTimeout.current = setTimeout(() => setShowCard(false), 250);
          }}
        >
          <button
            onClick={() => {
              const message = encodeURIComponent(
                "Hi, I want to know more about kingcompiler courses"
              );
              window.open(
                `https://wa.me/919038162791?text=${message}`,
                "_blank"
              );
            }}
            style={{
          ...
          {/* Hover Card - Only this disappears when X is clicked */}
          {showCard && showWhatsAppPopup && (
            <div
              style={{
                position: "absolute",
                bottom: "110%",
